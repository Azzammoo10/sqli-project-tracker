...
// L’API /projects actuelle n’envoie pas "client" objet mais "clientName" (string)
...
const getTypeLabel = (p: any) => p.typeLabel || p.type || '—';
...
const renderTypeBadge = (p: any) => {
    ...
    const t = (['Delivery','TMA','Interne'].includes(label) ? label : '—') as keyof typeof map;
    return (
        <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${map[t]}`}>
      {t}
    </span>
    );
};
...
// Initiales pour avatars à partir de username/nom
...
export default function ProjectsTable({
    ...
    const hasTypeInData = useMemo(
        () => projects.some(p => (p as any).type != null),
        [projects]
    );
        ...
        <div className="bg-white rounded-lg shadow">
            ...
            <div className="p-4 border-b grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                {hasTypeInData ? (
                    ...
                    <div /> /* placeholder pour conserver la grille en 3 colonnes */
                )}
            </div>
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex -space-x-2">
                                        {(p.developpeurs ?? []).slice(0, 5).map((d, i) => (
                                            <div
                                                key={`${d.username || d.email ||'dev'}-${i}`} // pas d'id dans le JSON

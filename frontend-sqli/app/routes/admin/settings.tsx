...
type Profile = {
  ...
  skills?: string[]; // ex. ['PMP', 'Agile']
  actif?: boolean;
  createdAt?: string;
  avatarUrl?: string;
  emails?: string[]; // multi-emails
  statusSince?: string; // pour le "1 month ago"
};
...
export default function AdminSettings() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const me = await authService.getCurrentUser(); // /auth/me
        // normalise un peu
        setUser({
          ...me,
          emails: [me.email],
  ...
  const handleLogout = async () => {
    try { await authService.logout(); window.location.href = '/auth/login'; }
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  />
                  <div>
                    <div className="text-lg font-semibold text-gray-900">{user.nom ? `${user.nom} ${user.prenom ?? ''}` : user.username}</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input className="mt-1 w-full px-3 py-2 rounded-md bg-gray-50 border border-gray-200 text-black" readOnly
                ...
                <div>
                  ...
                  <div className="mt-1 relative">
                    <input className="w-full px-3 py-2 rounded-md bg-gray-50 border border-gray-200 text-black" readOnly
                           value={user.gender ?? ''} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {(user.skills ?? []).map(s => <Chip key={s}>{s}</Chip>)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-1 relative">
                    <input className="w-full px-3 py-2 rounded-md bg-gray-50 border text-black border-gray-200" readOnly
              ...
              <div className="mt-8">
                ...
                <button
                  className="mt-4 px-3 py-2 rounded-md border border-gray-200 hover:bg-purple-950 text-black hover:text-white"
                  onClick={() => toast('Ajout d’email à implémenter')}
                >
                  + Add Email Address
                </button>
              </div>

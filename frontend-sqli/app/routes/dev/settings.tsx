...
const Field = ({ label, children }: { label: string; children: React.ReactNode }) => (
  <label className="block"><div className="text-xs font-medium text-gray-500 mb-1">{label}</div>{children}</label>
);
...
export default function DevSettings() {
  ...
  useEffect(() => { (async () => {
    try {
      ...
      setUser(enriched); setForm(enriched); origRef.current = enriched;
